...
def booking_data():
    ...
    blocked = Availability.objects.create(
        lsa=lsa,
...
def test_parent_history_filter_and_booking_detail(api_client, booking_data):
    ...
    assert history.data["data"][0]["booking_id"] == str(booking.id)
    assert detail.data["data"]["lsa"]["name"] == "Schedule LSA"
    assert detail.data["data"]["slot"]["start_time"] == time(10)
    assert detail.data["data"]["payment_status"] == Payment.Status.SUCCESS
...
def test_cancellation_releases_slot_and_dashboard_counts(api_client, booking_data):
    ...
    assert response.data["data"]["booking_status"] == Booking.Status.CANCELLED
    assert booked.status == Availability.Status.AVAILABLE
    assert summary.data["data"] == {"total_bookings": 1, "upcoming": 0, "completed": 0, "cancelled": 1, "failed": 0}
...
def test_schedule_returns_all_states_and_date_filter(api_client, booking_data):
    ...
    assert response.status_code == 200
    assert {item["id"] for item in response.data["data"]} == {str(available.id), str(booked.id)}
    assert {item["status"] for item in response.data["data"]} == {
        Availability.Status.AVAILABLE,
        Availability.Status.BOOKED,
    }
    assert str(blocked.id) not in {item["id"] for item in response.data["data"]}
